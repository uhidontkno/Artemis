...
export default class PrivacyCommand extends Command {
  async run(ctx: CommandContext) {
    ...
    try {
    // @ts-ignore
    config = atob(dbread(db,"config",ctx.guildId).value)
    } catch {}
...
Our verification bot is privacy respecting, so there is zero tracking, zero ads, zero fingerprinting. [We are even open source](https://github.com/uhidontkno/Artemis). We only collect and store data that is absolutely necessary for the functioning of our verification bot. Here is the breakdown:
* **Your IP address** is hashed using 2 numerical, non-cryptographical *(that means we do math)* hashes merged together. Therefore, we cannot reverse it. Additionally, we do not store your IP address if you have never had a successful verification.
...
* **Your server configuration** (if you have set up your server with us) is a simple JSON containing all of the paramaters that you or a server admin entered when running the \`/setup\`. This is stored in Base64.
